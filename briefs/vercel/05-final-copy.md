...
That’s it. That’s the guarantee.
...
A simple way to think about the boundaries:
...
- **Your job:** Configure your app correctly, manage your secrets, monitor what’s happening, and understand the behavior of everything your code calls.
- **Your AI API provider’s job:** Respond to the API calls your code makes and bill you for them.

When your Vercel function receives a request and calls an AI API, Vercel ran the function. The AI provider charges you for the call. Neither one is responsible for the other’s portion of the bill.
...
- *“It’s cheap or free right now, so there’s nothing to worry about.”*
...
This is where many beginner production problems start.
...
- If a variable is missing in production, your app may fail, return cryptic errors, or fall back to behavior you didn’t expect.
...
Check the current Vercel documentation on how environment variables work per environment. It’s straightforward once you’ve read it, but skipping it leads to the classic “it worked in preview but broke in production” problems.
...
Vercel’s plans include different allowances and behaviors for things like function usage, bandwidth, and execution time. If your app stays lightly used, you may stay well within those allowances. If traffic picks up — or if something in your app generates more requests than you expect — usage can grow faster than you realize.

For AI apps, there’s an extra meter running: **the expensive part is usually not Vercel.**

When your function calls an AI API:

- Vercel is responsible for executing your function and may meter that usage according to your plan.
- Your AI provider is responsible for processing the request and billing you for it.

These are separate systems and separate bills. A surge in traffic to your app means more Vercel function invocations *and* more AI API calls, each billed by its respective service.

Neither Vercel nor your AI provider will automatically protect you from every possible cost scenario. Different plans include different limits, caps, and behaviors when you exceed them. These details can change over time. Always check the current documentation and pricing pages for every service your app depends on. Don’t assume protections exist that you haven’t verified.
...
Vercel provides function logs and analytics features. These are not luxuries for advanced users — they are the minimum you need to operate a real app.
...
**Environment variables and secrets**  
As covered earlier, preview and production can have different environment variables. If you’ve been testing in preview with a test API key pointed at a sandbox database, and production has a different key pointed at real data — or a missing variable — behavior will differ the moment you promote.
...
- [ ] **Environment variables verified.** Every variable your app needs is set correctly in the production environment — not just in preview.
...
- [ ] **Preview URLs not shared with real users.** If you’ve been sharing a preview URL for testing, make sure users are directed to your production URL — not a preview that may behave differently or not persist.
...
Vercel does not know how many AI API calls are too many for you. Your AI provider may have tools to set usage or spending limits, but don’t rely on external services to enforce behavior your app should be controlling.
...
Vercel keeps a history of your deployments and lets you promote a previous deployment back to production. Find that feature in your dashboard before you need it. Click through the flow once so it’s familiar.

When something breaks in production — and at some point, something will — the difference between a stressful situation and a minor interruption is often just knowing where the rollback option is.
