...
- Ample white space to reduce cognitive load, but absolutely no artificial padding to inflate page count.
...
- **H2 (Main Sections):** Used for primary concepts. Do not force page breaks before every H2 simply to inflate the page count. Allow natural text flow.
...
- **Reader Exercises / Field Notes:** Use distinct "callout" boxes.
...
- **Sizing:** Maximum width of 80% of the page to ensure margins frame the image. Do not use illustrations simply to inflate page count.
- **Captions:** Every image must have a brief, italicized caption centered beneath it.

## 9. Honest Layout Requirements
Layout must be honest and strictly adhere to the booklet density standards. The following practices are strictly forbidden as they artificially inflate page count:
- No CSS page-count inflation.
- No forced page breaks before every H2.
- No excessive heading spacing.
- No artificially enlarged fonts.
- No excessive margins.
- No illustrations used solely to inflate page count.

## 10. Recommended PDF Export Structure
- **Hyperlinks:** All internal references and the Table of Contents must be fully clickable.
- **Bookmarks:** Generate PDF bookmarks (outline) automatically from H2 and H3 tags.
- **Metadata:** Ensure PDF properties (Title, Author, Subject) are correctly populated.

## 11. Recommended Output Folders and Filenames
Exports should be saved in each booklet's respective `dist/` directory.
- *Individual File Convention:* `<booklet-slug>-v1.0.pdf`

## 12. Individual Booklet Strategy
**Recommendation: Individual PDFs only.**
The current layout phase should produce individual PDFs only. These booklets function as modular pre-work and stage-specific support pieces.

## 13. Layout Risks in the Manuscripts
- **Orphans and Widows:** Long checklists or code blocks breaking awkwardly across page boundaries.
- **Markdown Inconsistencies:** Use of raw HTML or non-standard markdown tables.
- **Image Paths:** Ensure assets are correctly referenced.

## 14. Manuscript Formatting Cleanup Needed Before Layout
...
- **Verify Heading Levels:** Ensure no skipping.
- **Remove Meta-Text:** Strip out any lingering production notes.

## 15. The Double-Check Gate Before PDF Generation
No final PDFs may be generated until honest-layout proof generation, a density audit, and human visual review are complete.

**Layout Proof Review:**
Before generating the final `dist/` PDFs, a "Layout Proof" must be generated for visual inspection and audit. Reviewers must confirm:
- [ ] Manuscript word count
- [ ] Proof page count
- [ ] Words/page calculation
